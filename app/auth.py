...
def build_session_info(request: Request) -> dict[str, Any]:
    ...
    lti_launched = bool(session.get("lti_launched"))
    course_id = normalize_canvas_course_id(session.get("canvas_course_id") or config.CANVAS_COURSE_ID)

    if oauth_on and has_user_token:
        auth_mode = "oauth"
        canvas_api_source = "user_token"
    elif oauth_on and lti_launched and not has_user_token:
        auth_mode = "oauth_pending"
        canvas_api_source = "none"
    elif lti_launched:
        auth_mode = "lti_dev"
        canvas_api_source = "admin_token" if config.CANVAS_API_TOKEN else "none"
    elif config.CANVAS_API_TOKEN and not oauth_on:
        auth_mode = "direct_dev"
        canvas_api_source = "admin_token"
    else:
        auth_mode = "anonymous"
        canvas_api_source = "none"
    ...
    return {
        ...
        "launched_from_canvas": lti_launched,
        "auth_mode": auth_mode,
        "canvas_api_source": canvas_api_source,
        "oauth_required": oauth_on,
